...
-(void)sendMessage:(NSTimer*)timer{
    ...
    int i=rand();
    NSString * path = [[self.imapSession defaultNamespace] pathForComponents:@[@"TEST"]];
        //append message
        MCOMessageBuilder * builder = [[MCOMessageBuilder alloc] init];
        [[builder header] setFrom:[MCOAddress addressWithDisplayName:@"test" mailbox:@"me@me.com"]];
        NSMutableArray *to = [[NSMutableArray alloc] init];
        MCOAddress *newAddress = [MCOAddress addressWithMailbox:@"exmple@gmail.com"];
        [to addObject:newAddress];
        [[builder header] setTo:to];
        [[builder header] setSubject:@"SMTP Test"];
        [builder setHTMLBody:[@"SMTP Body Test " stringByAppendingFormat:@"%d",i]];
        NSDate *dt=[NSDate dateWithTimeIntervalSinceNow:-50000-(i*1000)];
        NSLog(@"%@",dt);
        [[builder header] setReceivedDate:dt];
        [[builder header] setDate:dt];
        NSData * rfc822Data = [builder data];
        MCOIMAPAppendMessageOperation *appendMsg=[self.imapSession appendMessageOperationWithFolder:path messageData:rfc822Data flags:0];
        [appendMsg start:^(NSError *error, uint32_t createdUID) {
            NSLog(@"%@:%d",error,createdUID);
        }];
        to=nil;
        builder=nil;
}
...
-(void)createFolder{
	...
	MCOIMAPFolderInfoOperation *imapFolderInfo = [self.imapSession folderInfoOperation:path];
	
	[imapFolderInfo start:^(NSError *error, MCOIMAPFolderInfo *info)
     {
         if (error) {
             NSLog(@"error:%@ %d %d",error,error.code,MCOErrorNonExistantFolder);
             if (error.code==MCOErrorNonExistantFolder) {
                 
                 MCOIMAPOperation *createOp = [self.imapSession createFolderOperation:path];
...
- (IBAction)dc:(id)sender {
    [self.imapSession.disconnectOperation start:nil];
